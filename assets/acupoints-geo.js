...
const ACUPOINTS = [
  ...
  { id:'si_quanliao', name:'顴髎', code:'SI18', meridian:'SI', side:'LR',
    ...
    /* v2.79修正：原式 y 取外眥→鼻翼之0.55、x 直接等於外眥，落點偏內上（約在
       眼眶下緣外側），比實際顴骨下緣高。顴髎應在「目外眥直下、顴骨下緣凹陷處」，
       故 y 下移至0.78（貼近顴弓／顴骨體下緣高度），x 再向外側偏0.05*sc
       （外眥寬約90mm ⇒ 約4–5mm），使其落在顴骨隆起外下方之凹陷。
       外側方向以外眥→顳點的x向量判定，不預設左右正負號。 */
    calc:(kp,s,sc)=>{
      const e = s==='R'?kp[IDX.eyeOuterR]:kp[IDX.eyeOuterL];
      const ala = s==='R'?kp[IDX.noseAlaR]:kp[IDX.noseAlaL];
      const tem = s==='R'?kp[IDX.templeR]:kp[IDX.templeL];
      const dir = Math.sign(tem.x - e.x) || (s==='R' ? -1 : 1);   // 向臉部外側
      const w = sc || Math.abs(tem.x - e.x) * 4;
      return { x: e.x + dir*w*0.05, y: mid(e, ala, 0.78).y };
    }},
